<?php
    ...
    function showDmBlog($limit){
        $sql = "select * from dmbaiviet where 1 order by id asc limit {$limit}";
        return result1(0,$sql);
    }
    function showBlogLuotXem($limit){
        $sql= "select * from baiviet where public = 1 order by luotxem desc limit {$limit}";
        return result1(0,$sql);
    }
    function showNameDmBlog($id){
    ...
    function getHangHoaTheoLoai($maloai,$pagenum,$pagesize){
        ...
        $sql = "SELECT * FROM baiviet WHERE iddm = ?"
    ...
    function demHangHoaTheoLoai($iddm){
        $sql = "select count(*) as sodong from baiviet where iddm = ?";
    ...
    function Page($TotalProduct, $CurrentPage,$PageSize,$BaseLink)
    {
        ...
        $linkNextQuery  = ROOT_URL.'/'.$BaseLink.'/'.$_GET['slug'].'-'.$_GET['maloai'].'/page-'.($NextQuery['Page']).'.html';
        $linkLastQuery  = ROOT_URL.'/'.$BaseLink.'/'.$_GET['slug'].'-'.$_GET['maloai'].'/page-'.($LastQuery['Page']).'.html';
        ...
        $linkPrevQuery  = ROOT_URL.'/'.$BaseLink.'/'.$_GET['slug'].'-'.$_GET['maloai'].'/page-'.($PrevQuery['Page']).'.html';
        $linkFirstQuery  = ROOT_URL.'/'.$BaseLink.'/'.$_GET['slug'].'-'.$_GET['maloai'].'/page-'.($FirstQuery['Page']).'.html';
        ...
        if ($CurrentPage <= $TotalPage && $TotalPage >= 1) // nếu page hiện tại nhỏ hơn hoặc bằng tổng số page và và tổng số page >=1
        {
            ...
            if ($CurrentPage > ($LimitPage / 2)) // nếu page hiện tại lớn hon 5/2 
            {
                ...
                $linkCurrentQuery  = ROOT_URL.'/'.$BaseLink.'/'.$_GET['slug'].'-'.$_GET['maloai'].'/page-'.($CurrentQuery['Page']).'.html';
